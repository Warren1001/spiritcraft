	...
	public ComplexSchematicDataEntry constructDataEntry(String line) {
		if(line.contains(",")) {
				...
				switch(key.toLowerCase()) {
					...
					case "n":
						...
						if(file.exists()) {
							...
							else if(value.endsWith(".schem")) schematic = worldManager.getSchematicManager().getSchematic(KFiles.getSimpleName(file));
			...
			if(file.exists()) {
				...
				else if(line.endsWith(".schem")) return new ComplexSchematicDataEntry(worldManager.getSchematicManager().getSchematic(KFiles.getSimpleName(file)));
