...
public class StabTask extends ChargeTask {
	
	private static final double RADIUS = 1.1;
	
	private long stopTime;
	
	public StabTask(SpiritPlayer player, int itemSlot, double duration, int updatesPerSecond) {
		super(player, itemSlot, duration, updatesPerSecond);
	}
	...
	@Override
	public void onStart() {
		owner.getPlayer().getWorld().playSound(owner.getLocation(), Sound.ZOMBIE_PIG_ANGRY, 5F, 0.1F);
	}
	...
	public void onStop() {
		...
		loc.getWorld().playSound(loc, Sound.ZOMBIE_PIG_HURT, 5F, 0.1F);
